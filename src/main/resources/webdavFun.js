 ...
 * @LastEditDate          : 2024-07-11 10:30:18                              *
...
function updateDmsDocumentList(data) {
  if (data.length === 0) {
    document.getElementById("docsInDmsList").innerHTML = "❗️❗️❗️No document in New Sage DMS Audros server!";
  } else {
...
function updateTLSDocumentList(data) {
  if (data.length === 0) {
    document.getElementById("docsInTLSList").innerHTML = "❗️❗️❗️No document in Old file server!";
  } else {
...
function uploadFile(file) {
  ...
  fetch("/Data/FileUpload", {
    ...
    .then((data) => {
      var notyf = new Notyf();
      if (data.success) {
        notyf.success({ message: data.msg, duration: 3000, dismissible: true, position: { x: "right", y: "top" } });
      } else {
        notyf.error({ message: data.msg, duration: 3000, dismissible: true, position: { x: "right", y: "top" } });
      }
    })
    .catch((error) => {
      alert(error);
