 ...
 * @LastEditDate          : 2024-12-13 12:48:53                                                                      *
...
public class AuthFilter extends OncePerRequestFilter {
    ...
    private void setBusinessPartner(HttpServletRequest req, HttpServletResponse res) {
        String bpCode = req.getParameter("bpCode");
        if (bpCode != null) {
            req.getSession().setAttribute("bpCode", bpCode);
            req.getSession().setAttribute("bpName", supplierService.getBusinessPartnerByCode(bpCode));
        }
    }

    private boolean authDocAccessByPath(HttpServletRequest req, HttpServletResponse res) {
        ...
        if (userFuncs.size() == 0) {
            ...
            if (allowRead) {
                ...
                return modifiableMethods.contains(method) ? forbidden(req, res) : allow(req, res);
            }
