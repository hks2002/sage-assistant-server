 ...
 * @LastEditDate          : 2024-12-09 19:48:37                                                                      *
...
public class DmsService {
    ...
    private static LoadingCache<String, String> dmsSessionCache = Caffeine
        ...
        .removalListener(
            (RemovalListener<String, String>) (Auth, value, cause) -> {
                 ...
                 */
                log.debug("[Dms] SessionId cache {} {} is removed, cause is {}", Auth, value, cause);
                doLogout(value);
            }
        )
        .build(
    ...
    private static byte[] getDocumentBytes(String id) {
        ...
        return HttpService.getFile(
            "http://192.168.10.64:4040/cocoon/viewDocument/ANY?FileID=" + id + "&UserName=TEMP&dsn=dmsDS&Client_Type=25"
        );
    }
    ...
    public static void downloadDmsDocs(List<Docs> docs, String docSavePath) {
        ...
        for (Docs doc : docs) {
            ...
            log.debug("[DOWNLOAD] {} from Dms server to {} ...", doc.getFile_name(), docSavePath);
            ...
            if (f.exists()) {
                ...
                if (f.length() > 0 && f.length() != 581) {
                    ...
                    log.info("[DOWNLOAD] {} from Dms server to {}, size {}", doc.getFile_name(), fileName, f.length());
                    // set file time
                    f.setLastModified(doc.getDoc_modified_at().getTime());
                } else {
                    // Log the download failure due to empty file
                    log.error("[DOWNLOAD] {} from Dms server to {}, size {}", doc.getFile_name(), fileName, f.length());
                    // delete
                    f.delete();
                }
            } else {
                // Log the download failure due to file not existing
                log.error("[DOWNLOAD] {} from Dms server to {}", doc.getFile_name(), fileName);
            }
        }
    }
}
