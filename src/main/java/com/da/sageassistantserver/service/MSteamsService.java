 ...
 * @LastEditDate          : 2024-12-18 17:42:04                                                                       *
...
public class MSTeamsService {
    ...
    public static void sendMessage(String workFlowUrl, JSONObject data) {
        ...
        if (idMap.get(workFlowUrl).tryConsume(1)) {
            HttpResponse<String> response = HttpService.request(workFlowUrl, "POST", data.toString());
            ...
            log.warn("WeWork request {} over limit {} per Min", workFlowUrl, REQUEST_LIMIT_PER_MIN);
        }
    }
     ...
     * @param message
     */
    public static void sendEmailMessage(String workFlowUrl, String title, String to, String message) {
        ...
        for (String paragraph : paragraphs) {
            ...
            for (String line : lines) {
                if (line.indexOf(":\t") > -1) {
                    ...
                    sb.append("<B>").append(Utils.withRightPad(factKV[0], 5, '\u3000')).append(":</B>"); // 全角空格
                    if (factKV.length == 2) {
                        sb.append(factKV[1]);
                        sb.append("<br>");
                    }
     ...
     * @param msg
     */
    public static void sendAdaptiveCardMessage(String workFlowUrl, String totalMessage) {
        Utils
