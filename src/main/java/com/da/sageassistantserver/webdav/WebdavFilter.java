 ...
 * @LastEditDate          : 2024-12-19 18:29:50                                                                      *
...
public class WebdavFilter implements Filter {
  ...
  final List<String> skipFiles = Arrays.asList("DESKTOP.INI", "AUTORUN.INI", "AUTORUN.INF");
  ...
  public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
    ...
    String fileName = Optional.ofNullable(StringUtils.getFilename(pathInfo)).orElse("");
    String fileExt = Optional.ofNullable(StringUtils.getFilenameExtension(fileName)).orElse("").toUpperCase();
    String fileNameNoExt = Optional.ofNullable(StringUtils.stripFilenameExtension(fileName)).orElse("");
    ...
    if (httpReq.getMethod().equals("GET") && waterMakerFileTypes.contains(fileExt)) {
      ...
      log.debug("resWrapper:{} --- len:{}", resWrapper.getStatus(), resWrapper.getBytes().length);
      if (resWrapper.getStatus() == HttpServletResponse.SC_NOT_MODIFIED && resWrapper.getBytes().length == 0) {
        return;
      }
      ...
      if (fileExt.equals("PDF")) {
        success = ITextTools.addTextFull(data, bosForWaterMark, wmText.toString(), 0.1f, 30);
      } else {
        ...
        if (ITextTools.toPdf(fileExt, data, bosForNewPdf)) {
          success = ITextTools.addTextFull(bosForNewPdf.toByteArray(), bosForWaterMark, wmText.toString(), 0.1f, 30);
        } else {
          success = false;
        }
      }
      ...
      if (success) {
        if (fileExt.equals("PDF")) {
          httpRes.addHeader("Content-Disposition", "inline; filename=\"" + outFileName + "\"");
        } else {
          httpRes.addHeader("Content-Disposition", "inline; filename=\"" + outFileName + ".PDF\"");
        }
