 ...
 * @LastEditDate          : 2024-12-09 19:14:06                                                                       *
...
public class LineNoteController {
    ...
    public JSONObject updateLineNote(
        ...
        if (rtn) {
            return ResponseJsonHelper.rtnObj(true, ResponseJsonHelper.MsgTyp.RESULT, "Update line note success");
        } else {
            return ResponseJsonHelper.rtnObj(false, ResponseJsonHelper.MsgTyp.ERROR, "Update line note failed.");
        }
    }
    ...
    public JSONObject deleteLineNote(
        @RequestHeader(value = "authorization", required = false) String Auth,
        @RequestParam(value = "NoteId", required = false, defaultValue = "") String NoteId
    ) {
        User user = userService.getUserByAuth(Auth);
        if (user == null) {
            return ResponseJsonHelper.missingAuth();
        ...
        if (lineNote == null) {
            return ResponseJsonHelper.rtnObj(false, ResponseJsonHelper.MsgTyp.RESULT, "Line note not found.");
        }
        ...
        if (rtn == 0) {
            return ResponseJsonHelper.rtnObj(true, ResponseJsonHelper.MsgTyp.RESULT, "Delete line note failed.");
        } else {
            return ResponseJsonHelper.rtnObj(false, ResponseJsonHelper.MsgTyp.ERROR, "Delete line note success");
        }
    }
}
