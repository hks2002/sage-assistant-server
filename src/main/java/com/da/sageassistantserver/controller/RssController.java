 ...
 * @CreatedDate           : 2022-03-26 20:03:00                                                                      *
 * @LastEditors           : Robert Huang<56649783@qq.com>                                                            *
 * @LastEditDate          : 2024-12-25 14:31:00                                                                      *
...
public class RssController {
  ...
  public Channel rss(
    ...
    channel.setItems(
      Collections.singletonList(getRssItemBySite(Site, type, range))
    );
    return channel;
  }
  ...
  private Item getRssItemBySite(String Site, String type, Integer range) {
    ...
    if (type.equals("") || type.equals("OrderLine")) {
      tobeDealWithOrderLineContent =
        rssService.checkTobeDealWithOrderLine(Site);
    }
    ...
    if (!tobeDeliveryContent.equals("")) {
      content +=
        "<h3>The product need to be delivery in next " +
        range +
        " Days:[Delivery]</h3>";
    ...
    if (!tobePurchaseBom.equals("")) {
      content += "<h3>These Boms need to be purchase:[PurchaseBom]</h3>";
      content += tobePurchaseBom;
    }
    if (!tobeReceiveContent.equals("")) {
      content +=
        "<h3>These Boms need to be receive in next " +
        range +
        " Days:[Receive]</h3>";
