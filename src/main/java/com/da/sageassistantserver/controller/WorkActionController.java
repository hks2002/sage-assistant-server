 ...
 * @LastEditDate          : 2024-12-09 19:15:25                                                                       *
...
public class WorkActionController {
    ...
    public List<WorkAction> getWorkAction(
        ...
        @RequestParam(value = "DateFrom", required = false, defaultValue = "2099-12-31") String DateFrom,
        @RequestParam(value = "DateTo", required = false, defaultValue = "1901-01-01") String DateTo
    ) {
        return workActionService.getWorkAction(Id, Site, Type, ProjectNO, User, DateFrom, DateTo);
    }
    ...
    public JSONObject updateWorkAction(
        ...
        if (!workAction.getCreate_by().equals(user.getId())) {
            return ResponseJsonHelper.rtnObj(false, MsgTyp.RESULT, "You are not the owner of this Work Action.");
        }

        boolean rtn = workActionService.updateWorkAction(workAction);

        if (rtn) {
            return ResponseJsonHelper.rtnObj(true, MsgTyp.RESULT, "Update work action success");
        } else {
            return ResponseJsonHelper.rtnObj(false, MsgTyp.ERROR, "Update work action failed.");
        }
    }
    ...
    public JSONObject deleteWorkAction(
        ...
        WorkAction workAction = workActionService.getWorkActionById(Long.valueOf(ActionId));

        if (workAction == null) {
            return ResponseJsonHelper.rtnObj(false, MsgTyp.RESULT, "Work Action not found.");
        }
        if (workAction.getCreate_by() != null && !workAction.getCreate_by().equals(user.getId())) {
            return ResponseJsonHelper.rtnObj(false, MsgTyp.RESULT, "You are not the owner of this Work Action.");
        }

        Integer rtn = workActionService.deleteWorkAction(Long.valueOf(ActionId));
        if (rtn.equals(0)) {
            return ResponseJsonHelper.rtnObj(false, MsgTyp.RESULT, "Delete Work Action success");
        } else {
            return ResponseJsonHelper.rtnObj(true, MsgTyp.ERROR, "Delete Work Action failed.");
        }
    }
}
